...
#     0       1  2      3          4       5    6   7        8
bottleDict = {"box": [["Bottle1", 610, 710, 30, 100],
                      ["Bottle3", 690, 730, 20, 110],
                      ["Bottle2", 600, 700, -50, 95],
                      ["Bottle1", 650, 700, 60, 100],
                      ["Bottle2", 650, 755, -10, 98],
                      ["Bottle1", 678, 650, -15, 98],
                      ["Bottle3", 680, 740, -25, 100],
                      ["Bottle1", 700, 760, 35, 100]], "ABC1": [1, 2, 3], "ABC2": [1, 2, 3]}
PI = 3.1415926
uArmbottom_P = [0, 0, 0]  # 机械臂基坐标系下，uArmbottom_P，单位mm
world2uarm_P = np.array([[300], [-400], [0]])  # 世界坐标系原点相对于机械臂基坐标系的位置矢量，单位mm
world2uarm_R = PI/2  # 世界坐标系绕z轴转90度后，与机械臂基坐标系重合，用来生成旋转矩阵
...
for i in range(0, numRow[0]):
    ...
    for j in range(0, 2):
        Px[i].append(float(uarm_P[0]))  # 将位置矢量(Z分量不使用)添加到矩阵
...
for i in range(0, numRow[0]):  # 依次从距离远近取瓶子操作
    ...
    bottomAngle0 = swift.get_servo_angle(servo_id=0)  # 获取底座关节角0,是否是以x轴正向为0°
    swift.set_position(50, Px[i][6], Px[i][7] + 50, speed=speed, wait=True, cmd='G0')  # 把瓶子移动到靠近边缘的轨道上空
    ...
    swift.set_position(50, Px[i][6], Px[i][7], speed=speed, wait=True, cmd='G0')  # 把瓶子移动到靠近边缘的轨道
