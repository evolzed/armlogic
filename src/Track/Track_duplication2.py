...
class Track:
    ...
    def createTarget(self, bottleDict,frame,featureimg,nFrame,_vision,_imgproc, preframeb,targetSignal=0,):
        """
        增加新的Target目标功能
        ...
        return targetDict, bottleDict, uuIDList, p0, label, centerLists, targetSignal, preframeb
...
if __name__ == "__main__":
    ...
    while True:
        ...
        if targetSignal == 1:
            ...
            # 创建target
            tempDict, tempBottleDict, uuIDList, p0, label, centerLists, targetSignal,preframeb = targetTracking.createTarget(dataDict, frame, featureimg,nFrame,_vision, _imgproc, preframeb,targetSignal=0,)
            # if centerLists is not None and len(centerLists) > 0:
            #     for seqN in range(len(centerLists)):
            #         cv2.circle(drawimg, (centerLists[seqN][0], centerLists[seqN][1]), 24, (0, 0, 255), 7)
            # if p0 is not None and label is not None:
