...
def work_thread(cam, pData, nDataSize):
    ...
    while True:
        # print(dir(pData))
        # pData.to_bytes(2, byteorder="big")
        ret = cam.MV_CC_GetOneFrameTimeout(pData, nDataSize, stFrameInfo, 1000)
        if ret == 0:
            # print(dir(stFrameInfo))
...
if __name__ == "__main__":
    ...
    import numpy as np
    import cv2
    print(data_buf._objects)
    barray = np.frombuffer(data_buf, dtype=np.ubyte)
    barray = barray.reshape(-1, 960)

    # img = cv2.imread(barray)
    cv2.imshow("img", barray)
    print(barray)
    try:
