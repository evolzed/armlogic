...
Public Class Multiple
    ...
    'Dim m_stNetTransInfo As MyCamera.MV_NETTRANS_INFO = New MyCamera.MV_NETTRANS_INFO
    ...
    Dim m_bDisplaying(4) As Boolean

    Dim m_bIsOpen As Boolean
    Dim m_bIsGrabbing As Boolean
    Dim m_nFrameCount(4) As UInteger
    Dim m_bIsSaveImage(4) As Boolean
    Dim m_CameraControlThread As New System.Threading.Thread(AddressOf CameraControlThread)
    Dim m_myCallback As MyCamera.cbOutputdelegate = New MyCamera.cbOutputdelegate(AddressOf cbOutputdelegateFunc)
    ...
    Private Sub cbOutputdelegateFunc(ByVal pData As IntPtr, ByRef pFrameInfo As MyCamera.MV_FRAME_OUT_INFO, ByVal pUser As IntPtr)
        ...
        If (m_bIsSaveImage(nIndex) = True) Then
            ...
            stSaveImageParam.enImageType = 1
    ...
    Private Sub CameraControlThread()

        While (m_bIsOpen And m_bIsGrabbing)
            Dim nCameraUsingNum As UInt32 = Convert.ToUInt32(TextBoxCameraUsingNum.Text)
            Dim i As UInt32 = 0
            For i = 0 To nCameraUsingNum - 1
    ...
    Private Sub Multiple_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        m_bDisplaying(0) = False
        m_bDisplaying(1) = False
        m_bDisplaying(2) = False
        m_bDisplaying(3) = False
    ...
    Private Sub ButtonInitCamera_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles ButtonInitCamera.Click
        ...
        For i = 0 To m_stDeviceInfoList.nDeviceNum - 1
            m_stDeviceInfo = CType(Marshal.PtrToStructure(m_stDeviceInfoList.pDeviceInfo(i), GetType(MyCamera.MV_CC_DEVICE_INFO)), MyCamera.MV_CC_DEVICE_INFO)
            nRet = dev(j).MV_CC_CreateDevice_NET(m_stDeviceInfo)
            If MyCamera.MV_OK <> nRet Then
            ...
            Else
                ...
                If m_stDeviceInfo.nTLayerType = MyCamera.MV_GIGE_DEVICE Then
                    ...
                    End If
                End If
                dev(j).MV_CC_RegisterImageCallBack_NET(m_myCallback, j)
    ...
    Private Sub ButtonStartGrabbing_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles ButtonStartGrabbing.Click
        ...
        For i = 0 To m_nCanOpenDeviceNum - 1
            ...
            End If
            m_bIsGrabbing = True
            nRet = dev(i).MV_CC_Display_NET(m_PictureBoxCamera(i).Handle)
            If MyCamera.MV_OK <> nRet Then
                MsgBox("Fail to display image")
            End If
            m_bDisplaying(i) = True
        Next
    ...
    Private Sub TimerCameraControl_Tick(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles TimerCameraControl.Tick
        ...
        For i = 0 To m_nCanOpenDeviceNum - 1
            ...
            nRet = dev(i).MV_CC_GetDeviceInfo_NET(stDevInfo)
