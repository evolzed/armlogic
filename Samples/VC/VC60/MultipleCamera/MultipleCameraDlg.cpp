...
BEGIN_MESSAGE_MAP(CMultipleCameraDlg, CDialog)
	...
	ON_BN_CLICKED(IDC_SAVE_IMAGE_BUTTON, OnBnClickedSaveImageButton)
	...
	ON_BN_CLICKED(IDC_SELECT_CAMERA1_CHECK, OnBnClickedSelectCamera1Check)
	ON_BN_CLICKED(IDC_SELECT_CAMERA2_CHECK, OnBnClickedSelectCamera2Check)
	ON_BN_CLICKED(IDC_SELECT_CAMERA3_CHECK, OnBnClickedSelectCamera3Check)
	ON_BN_CLICKED(IDC_SELECT_CAMERA4_CHECK, OnBnClickedSelectCamera4Check)
...
int CMultipleCameraDlg::SaveImage(void)
{
    ...
    for (int i = 0; i < m_nSelectDeviceNum; i++)
    {
        ...
        if (MV_OK == nRet)
        {
            ...
            {
                sprintf(chImageName, "Image_w%d_h%d_fn%03d.bmp", stImageInfo.nWidth, stImageInfo.nHeight, stImageInfo.nFrameNum);
            }
            else if (MV_Image_Jpeg == stParam.enImageType)
            {
                sprintf(chImageName, "Image_w%d_h%d_fn%03d.jpg", stImageInfo.nWidth, stImageInfo.nHeight, stImageInfo.nFrameNum);
            }
...
int CMultipleCameraDlg::ThreadFun(int nUsingDeviceNum)
{
    ...
    unsigned int nRecvBufSize = 0;
    nRet = m_pcMyCamera[nUsingDeviceNum]->GetIntValue("PayloadSize", &nRecvBufSize);
    if (nRet != MV_OK)
    {
        ShowErrorMsg(TEXT("[ThreadFun]failed in get PayloadSize"), nRet);
        nRecvBufSize = MAX_BUF_SIZE;
    }
	
    unsigned char * pData = (unsigned char *)malloc(sizeof(unsigned char) * nRecvBufSize);
    if (NULL == pData)
    {
        return STATUS_ERROR;
    }
    ...
    while(m_bStartGrabbing)
    {
        ...
        nRet = m_pcMyCamera[nUsingDeviceNum]->GetOneFrameTimeout(pData, &nDataLen, nRecvBufSize, &stImageInfo, 250);
        if (MV_OK == nRet)
        {
            nFrameCount = stImageInfo.nFrameNum;
        }
		
        if (0 == nUsingDeviceNum)
        {
            m_nLostFrame1Edit =  nLostFrame;;
            pos.Format(_T("%d"), m_nLostFrame1Edit);
            (GetDlgItem(IDC_LOST_FRAME1_EDIT))->SetWindowText(pos);
            pos.Format(_T("%d"), nFrameCount);
            (GetDlgItem(IDC_FRAME_COUNT1_EDIT))->SetWindowText(pos);
        }
        else if (1 == nUsingDeviceNum)
        {
            m_nLostFrame2Edit =  nLostFrame;;
            pos.Format(_T("%d"), m_nLostFrame2Edit);
            (GetDlgItem(IDC_LOST_FRAME2_EDIT))->SetWindowText(pos);
            pos.Format(_T("%d"), nFrameCount);
            (GetDlgItem(IDC_FRAME_COUNT2_EDIT))->SetWindowText(pos);
        }
        else if (2 == nUsingDeviceNum)
        {
            m_nLostFrame3Edit =  nLostFrame;;
            pos.Format(_T("%d"), m_nLostFrame3Edit);
        ...
        else
        {
            ...
            pos.Format(_T("%d"), m_nLostFrame4Edit);
            (GetDlgItem(IDC_LOST_FRAME4_EDIT))->SetWindowText(pos);
            pos.Format(_T("%d"), nFrameCount);
            (GetDlgItem(IDC_FRAME_COUNT4_EDIT))->SetWindowText(pos);
        }
    }
