...
Status CHalconGrabImageDlg::SetTriggerMode(void)
{
    try
    {
        set_framegrabber_param(m_htupHalconHandle, "TriggerMode", m_nTriggerMode);
    }
    catch(HException &except)
    {
        Herror  error_num = except.err;
...
Status CHalconGrabImageDlg::SetExposureTime(void)
{
    ...
    try
    {
        set_framegrabber_param(m_htupHalconHandle, "ExposureMode", MV_EXPOSURE_MODE_TIMED);
        set_framegrabber_param(m_htupHalconHandle, "ExposureAuto", MV_EXPOSURE_AUTO_MODE_OFF);
...
Status CHalconGrabImageDlg::SetGain(void)
{
    try
    {
        set_framegrabber_param(m_htupHalconHandle, "GainAuto", 0);
...
Status CHalconGrabImageDlg::GetTriggerSource(void)
{
    ...
    if ((unsigned int)MV_TRIGGER_SOURCE_SOFTWARE == (unsigned int)str[0].L())
    {
...
Status CHalconGrabImageDlg::SetTriggerSource(void)
{
    ...
    if (m_bSoftwareTriggerCheck)
    {
        ...
        try
        {
            set_framegrabber_param(m_htupHalconHandle, "TriggerSource", m_nTriggerSource);
        }
        catch(HException &except)
        {
