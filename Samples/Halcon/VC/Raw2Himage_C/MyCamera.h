...
public:
    ...
    // 保存图片
    int     SaveImage(MV_SAVE_IMAGE_PARAM* pstParam);

    // 注册图像数据回调
    int     RegisterImageCallBack(void(__stdcall* cbOutput)(unsigned char * pData, MV_FRAME_OUT_INFO* pFrameInfo, void* pUser),
                                    void* pUser);

    // 注册RGB图像数据回调
    int     RegisterImageCallBackForRGB(void(__stdcall* cbOutput)(unsigned char * pData, MV_FRAME_OUT_INFO_EX* pFrameInfo, void* pUser),
                                    void* pUser);
    // 注册消息异常回调
    ...
    //传出m_hDevHandle,供格式转换使用
    void*     GetDevHandle(/*void *handle*/);
private:
