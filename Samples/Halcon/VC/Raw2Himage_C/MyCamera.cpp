...
// 主动获取一帧图像数据
int     CMyCamera::GetOneFrame(unsigned char* pData, unsigned int* pnDataLen, 
                               unsigned int nDataSize, MV_FRAME_OUT_INFO* pFrameInfo)
{
    if (   NULL == m_hDevHandle 
        || NULL == pData 
        || NULL == pnDataLen 
    ...
    *pnDataLen  = 0;
    ...
    int nRet = MV_CC_GetOneFrame(m_hDevHandle, pData, nDataSize, pFrameInfo);
    if (MV_OK != nRet)
    {
        return MY_FAIL;
    }

    *pnDataLen = pFrameInfo->nWidth * pFrameInfo->nHeight * (((pFrameInfo->enPixelType >> 16) & 0x00ff) >> 3);
...
int     CMyCamera::SaveImage(MV_SAVE_IMAGE_PARAM* pstParam)
{
    if (NULL == pstParam)
    {
        return MY_FAIL;
    ...
    return MV_CC_SaveImage(pstParam);
}
                                         ...
                                         void* pUser)
{
    ...
    return MV_CC_RegisterImageCallBack(m_hDevHandle, cbOutput, pUser);
}

// 注册图像数据回调
int     CMyCamera::RegisterImageCallBackForRGB(void(__stdcall* cbOutput)(unsigned char * pData, MV_FRAME_OUT_INFO_EX* pFrameInfo, void* pUser),
                                         void* pUser)
{
    if (NULL == m_hDevHandle)
    {
...
//返回句柄供格式转换用
void* CMyCamera::GetDevHandle(/*void **handle*/)
{
