...
void CRaw2Himage_CDlg::OnBnClickedOpenButton()
{
    ...
    {
        m_pcMyCamera->GetIntValue("Width", &nWidth);
        m_pcMyCamera->GetIntValue("Height", &nHeight);
        //m_pImageBuffer = (unsigned char *)malloc(sizeof(unsigned char) * nWidth * nHeight * 3);
        //if (NULL == m_pImageBuffer)
        //{
        //    MessageBox(TEXT("malloc Fail"), TEXT("warning"), MB_OK | MB_ICONWARNING);
        //    return;
        //}
    }
...
unsigned int CRaw2Himage_CDlg::WorkThread(void* pUser)
{
    ...
    void *pHandle = pBasicDemo->m_pcMyCamera->GetDevHandle();
    if (NULL == pHandle)
    {
        return -1;
    }
    ...
    nRet = MV_CC_GetIntValue(pHandle, "PayloadSize", &stParam);
    if (MV_OK != nRet)
    {
        return -1;
    }
    ...
    {
        nRet = MV_CC_GetOneFrameTimeout(pHandle, pData, nDataSize, &stImageInfo, 1000);
        if(MV_OK == nRet)
        {
            ...
            if(IsColorPixelFormat(stImageInfo.enPixelType))//Color
            {
                ...
                {
                    nRet = ConvertToRGB(pHandle, pData , stImageInfo.nHeight, stImageInfo.nWidth, stImageInfo.enPixelType, pBasicDemo->m_pImageBuffer);
                    if (MV_OK != nRet)
                    {
                        return -1;
                    }
                ...
                nRet = ConvertRGBToHalcon(&Hobj, stImageInfo.nHeight, stImageInfo.nWidth, stImageInfo.enPixelType, pTemp, pDataSeparate);
                if (MV_OK != nRet)
                {
                    return -1;
                }
            ...
            else if(IsMonoPixelFormat(stImageInfo.enPixelType))//Mono
            {
                ...
                {
                    nRet = ConvertToMono8(pHandle, pData, pBasicDemo->m_pImageBuffer, stImageInfo.nHeight, stImageInfo.nWidth, stImageInfo.enPixelType);
                    if (MV_OK != nRet)
                    {
                        return -1;
                    }
                ...
                nRet = ConvertMono8ToHalcon(&Hobj, stImageInfo.nHeight, stImageInfo.nWidth, stImageInfo.enPixelType, pTemp);
                if (MV_OK != nRet)
                {
                    return -1;
                }
